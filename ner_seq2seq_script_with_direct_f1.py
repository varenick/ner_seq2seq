...
for run in range(num_runs):
    ...
    for step in range(num_steps):        
        ...
        if reinforce_strategy in ['argmax_advantage', 'direct']:
...
train_f1s_mean = np.mean(train_f1s, axis=0)
train_f1s_std = np.std(train_f1s, axis=0)
...
for name in ['train_losses', 'train_f1s', 'eval_losses', 'eval_f1s']:
    for infix in ['', '_noisy']:
        if name in ['train_losses', 'train_f1s'] and infix == '_noisy':
            continue
